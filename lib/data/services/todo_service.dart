...
class TodoService {
  ...
  // GET /todos?search=&page=&perPage=  (dengan paginasi)
  ...
  // GET /todos?perPage=9999  (untuk statistik Home)
  Future<ApiResponse<List<TodoModel>>> getAllTodos({
    required String authToken,
  }) async {
    final uri =
    Uri.parse('${ApiConstants.baseUrl}${ApiConstants.todos}')
        .replace(queryParameters: {'perPage': '9999'});
  ...
  Future<ApiResponse<TodoModel>> getTodoById({
    ...
    if (response.statusCode == 200) {
      final todo = TodoModel.fromJson(
        (body['data'] as Map<String, dynamic>)['todo']
  ...
  Future<ApiResponse<void>> updateTodo({
    ...
    if (response.statusCode == 200) {
      return ApiResponse(
          success: true, message: body['message'] as String);
    }
    return ApiResponse(success: false, message: _parseError(response));
  }
